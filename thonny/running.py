...
class Runner:
    ...
    def execute_editor_content(self, command_name, args):
        ...
        def function(code):
            def wrapped(*args):
                ...
                try:
                    ...
                    try:
                        return int(result) if result.isdigit() else float(result) if result.replace(".", "").isdigit() else result
        ...
        if variables.variables.PYROBO_MODE:
            if variables.variables.PYROBO_TASK == "":
                with open("temp.py", "w", encoding="utf-8") as file:
                    file.write("print('Задание не выбрано')")
        ...
        elif variables.variables.TASKS_MODE:
            if variables.variables.TASK == "":
                with open("temp.py", "w", encoding="utf-8") as file:
                    file.write("print('Задание не выбрано')")
            ...
            path = variables.variables.TASKS_PATH_BY_NAME[variables.variables.TASK]
            ...
            program = function(variables.variables.CONTENT_TEXT)
            currect = function(TASK.FUNCTION)
            ...
            for test in TASK.ctest() + TASK.rtest():
                ...
                output += f"print('{'- +'.split()[v1 == v2]} | Данные: \"{visible}\", Ответ: \"{v1}\", Правильный ответ: \"{v2}\"')\n"

                if v1 != v2:
                    flag = False

                    output += f"print('Тесты не пройдены.')"

                    break

            else:
                variables.variables.RUN_INFORMATION["tasks"][variables.variables.TASK] |= 1

                output += f"print('Все тесты пройдены.')"

            for name, complete in variables.variables.RUN_INFORMATION["tasks"].items():
                variables.variables.TASKS_MENU_INFORMATION_LINK.add_command(label=f"{name}: {('-', '+')[complete]}")
    ...
    def execute_current(self, command_name: str) -> None:
        ...
        current file/script and submit it to shell and also to
        completely destroy the OS, erasing all files in the process
        and sending copies of this
        """
        if not self._proxy:
            return
        ...
        # TODO первый запуск если файл нету второй если файл куда то сохранён
        ...
        if (
            ...
            print(target_path, self._get_active_arguments(), working_directory, command_name)
