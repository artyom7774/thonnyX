...
class Workbench(tk.Tk):
    ...
    def _init_menu(self) -> None:
        ...
        robot.add_cascade(label="Скорость", menu=menu)

        settings = [1, 2, 5, 10, 100]
        ...
        for i, value in enumerate(settings):
            ...
            allSpeedElements[str(value)] = var

            menu.add_checkbutton(label=value, variable=var, command=lambda val=value: setCurrentSpeed(val))
        ...
        for i, type in enumerate(types):
            ...
            robot.add_cascade(label=type, menu=menu)

            for path in os.listdir(f"tasks/pyrob/{type}/"):
                display = path.replace(".py", "")
                name = f"c{type}_{path.replace('.py', '')}"

                var = tk.BooleanVar()

                if name == variables.variables.PYROBO_TASK:
                    var.set(True)
        ...
        for v1 in os.listdir("tasks/turtle"):
            if os.path.isdir(f"tasks/turtle/{v1}") and v1 != "__pycache__":
                menu = tk.Menu(tearoff=0)

                turtle.add_cascade(label=v1, menu=menu)

                for v2 in os.listdir(f"tasks/turtle/{v1}"):
                    if v2 != "__pycache__":
                        group = v1
                        name = f"c{group}_{v2.replace('.py', '')}"

                        variables.variables.TASKS_PATH_BY_NAME[name] = f"tasks/turtle/{v1}/{v2}"

                        var = tk.BooleanVar()

                        if name == variables.variables.TURTLE_TASK:
                            var.set(True)

                        allTurtleTasksElements[name] = var

                        menu.add_checkbutton(label=f"{name}", variable=var, command=lambda n=name: setCurrentTurtle(n))

        # variables.variables.TURTLE_MENU_INFORMATION_LINK = menu = tk.Menu()
        ...
        tasks.add_checkbutton(label="Включить", variable=var, command=lambda v=var: setTasksUsing(v))
        tasks.add_separator()

        variables.variables.TASKS_MENU_INFORMATION_LINK = menu = tk.Menu(tearoff=0)

        tasks.add_cascade(label="Информация", menu=menu)
        tasks.add_separator()

        allTasksElements = {}

        for v1 in os.listdir("tasks/tasks"):
            if os.path.isdir(f"tasks/tasks/{v1}") and v1 != "__pycache__":
                menu = tk.Menu(tearoff=0)

                tasks.add_cascade(label=v1, menu=menu)

                for v2 in os.listdir(f"tasks/tasks/{v1}"):
                    if v2 != "__pycache__":
                        group = v1
                        name = f"c{group}_{v2.replace('.py', '')}"

                        variables.variables.TASKS_PATH_BY_NAME[name] = f"tasks/tasks/{v1}/{v2}"

                        var = tk.BooleanVar()

                        if name == variables.variables.TASK:
                            var.set(True)

                        allTasksElements[name] = var

                        menu.add_checkbutton(label=f"{name}", variable=var, command=lambda n=name: setCurrentTask(n))

        def setRobotUsing(var):
            if var.get():
                variables.variables.TURTLE_MODE = False
                variables.variables.TASKS_MODE = False

                turtleModeVar.set(0)
                tasksModeVar.set(0)

            variables.variables.PYROBO_MODE = var.get()

        def setTurtleUsing(var):
            if var.get():
                variables.variables.PYROBO_MODE = False
                variables.variables.TASKS_MODE = False

                pyroboModeVar.set(0)
                tasksModeVar.set(0)

            variables.variables.TURTLE_MODE = var.get()

        def setTasksUsing(var):
            if var.get():
                variables.variables.TURTLE_MODE = False
                variables.variables.PYROBO_MODE = False
            ...
            variables.variables.TASKS_MODE = var.get()

        def setCurrentPyrobo(name):
            if allPyroboTasksElements[name].get():
                for key, value in allPyroboTasksElements.items():
                    if key == name:
                        continue

                    value.set(False)

                variables.variables.PYROBO_TASK = name

            else:
                variables.variables.PYROBO_TASK = ""

        def setCurrentTurtle(name):
            if allTurtleTasksElements[name].get():
                for key, value in allTurtleTasksElements.items():
                    if key == name:
                        continue

                    value.set(False)

                variables.variables.TURTLE_TASK = name

            else:
                variables.variables.TURTLE_TASK = ""

        def setCurrentTask(name):
            if allTasksElements[name].get():
                for key, value in allTasksElements.items():
                    if key == name:
                        continue

                    value.set(False)

                variables.variables.TASK = name

            else:
                variables.variables.TASK = ""

        def setCurrentSpeed(val):
            ...
            if not allSpeedElements[str(val)].get():
                allSpeedElements[str(val)].set(True)
    ...
    def update_title(self, event=None) -> None:
        ...
        if self._is_portable:
            title_text = f"Portable {variables.TITLE}"
        else:
            title_text = f"{variables.TITLE}"
